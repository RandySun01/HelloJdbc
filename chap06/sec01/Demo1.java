...
private static DbUtil dbUtil=new DbUtil();
	...
	private static int addBook(Book book)throws Exception{
		...
		pstmt.setFloat(2, book.getPrice());  // 给第二个空设置值
		pstmt.setString(3, book.getAuthor()); // 给第三个空设置值
		pstmt.setInt(4, book.getBookTypeId());  // 给第四个空设置值
		File context=book.getContext(); // 获取文件
		InputStream inputStream=new FileInputStream(context);
		pstmt.setAsciiStream(5, inputStream,context.length());  // 给第五个空设置值
	...
	public static void main(String[] args) throws Exception{
		...
		Book book=new Book("helloWorld", 100, "阿志", 1,context);
